...
class thresh_Segmentation:
    ...
    def imgAdaptThresh(self):
        ...
        thresh1 = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 25, 10)
        thresh2 = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 25, 10)
    ...
    def OtsuThresh(self):
        img = cv.imread(os.path.abspath(os.path.join(__file__, self.path)), 0)
        threshod = 145
        ret, thresh = cv.threshold(img, threshod, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
        images = [img, thresh]
        self.show(images)
    ...
    def triangle(self):
        img = cv.imread(os.path.abspath(os.path.join(__file__, self.path)), 0)
        threshod = 145
        ret, thresh = cv.threshold(img, threshod, 255, cv.THRESH_BINARY + cv.THRESH_TRIANGLE)
        images = [img, thresh]
        self.show(images)
